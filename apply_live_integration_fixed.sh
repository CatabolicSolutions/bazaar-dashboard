            ...
            for j in range(i-10, i):
                if "'side':" in lines[j]:
                    ...
                    if 'buy' in side_raw:
                        ...
                        side = 'sell'
                    break
            # Build notes string separately to avoid escaping issues
            notes = 'Auto trade ' + str(ready['intent_id'])
            log_line = ' ' * indent + f'log_trade("tradier", candidate["symbol"], "{side}", args.qty, limit_price, notes="{notes}")\n'
