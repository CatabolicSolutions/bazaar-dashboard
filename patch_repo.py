...
Usage: python3 patch_repo.py /path/to/repo
"""
...
def patch_tradier_auto_trade(root):
    ...
    # Find line with 'committed = service.record_commit(ready, broker_response)'
    for i, line in enumerate(lines):
...
def patch_health_check(root):
    path = os.path.join(root, 'bazaar_scripts/tradier_health_check.sh')
    if not os.path.exists(path):
        # maybe in different location
        alt = os.path.join(root, '../bazaar_scripts/tradier_health_check.sh')
        if os.path.exists(alt):
            path = alt
        else:
            print(f"ERROR: health_check.sh not found")
            return False
    backup(path)
    with open(path, 'r') as f:
        lines = f.readlines()
...
def main():
    ...
    # Risk controls already updated; skip
    
    if not patch_tradier_auto_trade(root):
        success = False
    if not patch_health_check(root):
        success = False
