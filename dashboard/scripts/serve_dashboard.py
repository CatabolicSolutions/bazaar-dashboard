...
<div class="next-action"><div class="num">3</div><div class="action-copy"><strong>Preserve capital discipline</strong>Stay flat until setup quality is real.</div></div>'''
        ...
        primary_action_html = f'''<div class="drawer-card"><div class="drawer-head"><strong>Primary next action</strong><span class="pill">NOW</span></div><div class="drawer-body"><div style="font-size:18px;font-weight:800;line-height:1.1;margin-bottom:8px;">{esc((next_actions_data[0].get('title') if next_actions_data else 'Stabilize operator truth') if next_actions_data else 'Stabilize operator truth')}</div><div class="micro" style="margin-bottom:12px;">{esc((next_actions_data[0].get('detail') if next_actions_data else 'Force a fresh systems read, then act on the blocker.') if next_actions_data else 'Force a fresh systems read, then act on the blocker.')}</div><div class="toolbar"><button class="toolbtn" type="button" onclick="runCommand('refresh_truth')">Run primary action</button><button class="toolbtn" type="button" onclick="showView('overview')">Focus workspace</button></div></div></div>'''
