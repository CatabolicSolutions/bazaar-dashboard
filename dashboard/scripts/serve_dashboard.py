...
<div class="next-action"><div class="num">3</div><div class="action-copy"><strong>Preserve capital discipline</strong>Stay flat until setup quality is real.</div></div>'''
        ...
        compact_style = '''<style>body{padding:0;background:radial-gradient(circle at top,#132238 0%,#0b0f14 55%)}.shell{max-width:100%;padding:12px;gap:10px}.hero{padding:10px;border-radius:18px;background:linear-gradient(135deg,rgba(18,28,45,.96),rgba(10,15,24,.98));position:sticky;top:0;z-index:20;backdrop-filter:blur(12px)}.hero-top{align-items:center}.eyebrow{color:#5eead4}.subline{font-size:12px;max-width:none}.status-strip{grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.pill-card{padding:8px;border-radius:14px;background:rgba(255,255,255,.03)}.pill-value{font-size:15px}.pill-sub{font-size:10px;line-height:1.35}.decision{padding:10px;border-radius:16px;background:linear-gradient(135deg,rgba(56,189,248,.12),rgba(56,189,248,.03));display:grid;grid-template-columns:1.35fr .9fr;gap:10px;align-items:start}.decision-head{margin-bottom:0}.decision-title{font-size:20px}.decision-copy{font-size:14px;font-weight:700}.decision-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.mini-stat{padding:8px;border-radius:12px}.mini-stat .value{font-size:13px}.top-controls{display:grid;grid-template-columns:1.25fr .75fr;gap:10px;position:sticky;top:150px;z-index:18}.grid{grid-template-columns:1fr;gap:10px}.stack{gap:10px}.cockpit-main{display:grid;grid-template-columns:1.1fr .9fr;gap:10px;align-items:start}.section{padding:10px;border-radius:16px}.section-title{font-size:12px}.section-sub,.meta-row,.ops-summary,.micro-status{font-size:10px}.ops-list{gap:8px}.ops-item,.position-row,.activity-row,.next-action,.dense-card{padding:8px;border-radius:12px}.toolbar{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px}.toolbtn{padding:10px 8px;font-size:12px;border-radius:12px;transition:transform .14s ease,background .18s ease,border-color .18s ease,box-shadow .18s ease}.toolbtn:hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(0,0,0,.25)}.toolbtn.busy{opacity:.7;pointer-events:none}.kv{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.kv div{padding:6px;border-radius:10px}.dense-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.compact-pre{white-space:pre-wrap;max-height:120px;overflow:auto;background:#0f172a;padding:8px;border-radius:10px;border:1px solid #22304a;font-size:10px}.slim-input,.slim-textarea{width:100%;background:#0f172a;color:#e5edf7;border:1px solid #22304a;border-radius:10px;padding:8px}.slim-textarea{min-height:140px;font-family:monospace}.drawer{display:none}.drawer.open{display:block}.action-rail{display:grid;grid-template-columns:1.2fr .8fr;gap:10px}.primary-cta{padding:12px;border-radius:16px;border:1px solid rgba(94,234,212,.24);background:linear-gradient(135deg,rgba(45,212,191,.12),rgba(15,23,42,.92))}.primary-cta h2{font-size:22px;margin-bottom:6px}.primary-cta p{font-size:12px;color:var(--text-dim)}.engine-strip{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.engine-tile{padding:10px;border-radius:14px;background:rgba(255,255,255,.03);border:1px solid rgba(148,163,184,.14)}.engine-k{font-size:10px;color:var(--text-dim);text-transform:uppercase;letter-spacing:.08em}.engine-v{font-size:16px;font-weight:800;margin-top:4px}.accordion-head{display:flex;justify-content:space-between;align-items:center;cursor:pointer}.accordion-body{display:none;margin-top:8px}.accordion.open .accordion-body{display:block}.status-chip{display:inline-flex;padding:6px 10px;border-radius:999px;font-size:11px;font-weight:800;background:rgba(245,158,11,.16);border:1px solid rgba(245,158,11,.2)}@media (max-width:1100px){.status-strip,.decision,.decision-grid,.top-controls,.toolbar,.kv,.dense-grid,.action-rail,.engine-strip,.cockpit-main{grid-template-columns:1fr}.hero,.top-controls{position:static}}</style>'''
        controls_html = '''<section class="card section"><div class="section-head"><div><div class="section-title">Operator controls</div><div class="section-sub">Fast actions</div></div></div><div class="toolbar"><button class="toolbtn" data-command="refresh_truth" data-label="Refresh" type="button" onclick="runCommand('refresh_truth')">Refresh</button><button class="toolbtn" data-command="diagnose_tradier" data-label="Tradier diag" type="button" onclick="runCommand('diagnose_tradier')">Tradier diag</button><button class="toolbtn" data-command="diagnose_bloc" data-label="Bloc diag" type="button" onclick="runCommand('diagnose_bloc')">Bloc diag</button><button class="toolbtn" data-command="pause_bloc" data-label="Pause Bloc" type="button" onclick="runCommand('pause_bloc')">Pause Bloc</button><button class="toolbtn" data-command="resume_bloc" data-label="Resume Bloc" type="button" onclick="runCommand('resume_bloc')">Resume Bloc</button><button class="toolbtn danger" data-command="close_all" data-label="Close all" type="button" onclick="runCommand('close_all')">Close all</button></div></section>'''
        notes_html = '''<section class="card section"><div class="section-head"><div><div class="section-title">Operator note</div><div class="section-sub">Quick note, quick save</div></div></div><div class="ops-item"><textarea id="hq-note-input" class="slim-input" style="min-height:72px;"></textarea><div class="toolbar" style="margin-top:8px;"><button class="toolbtn" type="button" onclick="saveNote()">Save note</button><button class="toolbtn" type="button" onclick="loadOverview()">Reload</button></div><div id="note-status" class="micro-status">No note saved yet.</div></div></section>'''
        config_html = '''<section class="card section"><div class="section-head"><div><div class="section-title">Safety config</div><div class="section-sub">Edit limits directly</div></div></div><div class="ops-item"><textarea id="hq-config-input" class="slim-textarea" style="min-height:180px;"></textarea><div class="toolbar" style="margin-top:8px;"><button class="toolbtn" type="button" onclick="saveConfig()">Save config</button></div><div id="config-status" class="micro-status">Config not loaded yet.</div></div></section>'''
        logs_html = '''<section class="card section"><div class="section-head"><div><div class="section-title">Desk telemetry</div><div class="section-sub">Logs, refresh, feedback, signals</div></div></div><div class="dense-grid"><div class="dense-card"><h3>Refresh status</h3><pre id="refresh-status-box" class="compact-pre">--</pre></div><div class="dense-card"><h3>Action feedback</h3><pre id="action-feedback-box" class="compact-pre">--</pre></div><div class="dense-card"><h3>Recent notes</h3><pre id="notes-log-box" class="compact-pre"></pre></div><div class="dense-card"><h3>Signals</h3><pre id="signals-log-box" class="compact-pre"></pre></div><div class="dense-card"><h3>Trades/logs</h3><pre id="trades-log-box" class="compact-pre"></pre></div><div class="dense-card"><h3>Refresh log</h3><pre id="refresh-log-box" class="compact-pre"></pre></div></div></section>'''
...
<head>
    ...
    <title>HQ — Bazaar Operator Cockpit</title>
...
<body>
    <div class="shell">
        ...
        <section class="card hero">
            <div class="hero-top">
                <div>
                    ...
                    <div class="subline">Fast command surface for execution, diagnosis, and intervention.</div>
                </div>
            ...
            <div class="action-rail">
                <div class="primary-cta">
                    ...
                    <h2>{esc(primary_directive)}</h2>
                    <p>{esc(operator_summary)}</p>
                    <div class="micro-status" id="command-status">No command run yet.</div>
                </div>
                <div class="engine-strip">
                    <div class="engine-tile"><div class="engine-k">Tradier</div><div class="engine-v">{esc(tradier_status)}</div><div class="micro-status">{esc((money(tradier_buying_power) + ' ready') if tradier_status == 'Ready' else (tradier.get('top_blocker') or 'blocked'))}</div></div>
                    <div class="engine-tile"><div class="engine-k">Bloc</div><div class="engine-v">{esc('Active' if status == 'holding_active_inventory' else 'Idle')}</div><div class="micro-status">{esc(holding_asset + ' ' + str(holding_units) if status == 'holding_active_inventory' and holding_asset else money(deployable) + ' deployable')}</div></div>
                    <div class="engine-tile"><div class="engine-k">Freshness</div><div class="engine-v">{esc(age_label(tradier_age))}</div><div class="micro-status">Bloc {esc(age_label(bloc_age))}</div></div>
                </div>
            </div>
        </section>
        ...
        <div class="cockpit-main">
            <div class="stack">
                <section class="card section accordion open">
                    <div class="accordion-head" onclick="toggleSection(this)"><div><div class="section-title">Action queue</div><div class="section-sub">Immediate tasks, not theory</div></div><span class="status-chip">LIVE</span></div>
                    <div class="accordion-body"><div class="next-action-list">{next_actions_html}</div></div>
                </section>
                <section class="card section accordion">
                    <div class="accordion-head" onclick="toggleSection(this)"><div><div class="section-title">Engine truth</div><div class="section-sub">Readiness and exposure</div></div><span class="status-chip">3</span></div>
                    <div class="accordion-body"><div class="ops-list"><div class="ops-item"><div class="ops-item-head"><div class="ops-name">Tradier</div><div class="decision-badge badge-amber">{esc(tradier_status.upper())}</div></div><div class="kv"><div><span>Status</span>{esc(tradier_status)}</div><div><span>Buying power</span>{money(tradier_buying_power)}</div><div><span>Freshness</span>{esc(age_label(tradier_age))}</div></div></div><div class="ops-item"><div class="ops-item-head"><div class="ops-name">Bloc</div><div class="decision-badge badge-amber">{esc(str(live.get('mode') or 'live').upper())}</div></div><div class="kv"><div><span>State</span>{esc(status)}</div><div><span>Holding</span>{esc(holding_asset + (' ' + str(holding_units) if holding_units is not None and holding_asset else ''))}</div><div><span>Deployable</span>{money(deployable)}</div></div></div><div class="ops-item"><div class="ops-item-head"><div class="ops-name">Positions</div><div class="decision-badge badge-amber">{esc(str(active_positions))}</div></div><div class="positions-list">{positions_html}</div></div></div></div>
                </section>
            </div>
            <div class="stack">
                <section class="card section accordion open">
                    <div class="accordion-head" onclick="toggleSection(this)"><div><div class="section-title">Notes + config</div><div class="section-sub">Quick write, quick change</div></div><span class="status-chip">EDIT</span></div>
                    <div class="accordion-body">{notes_html}{config_html}</div>
                </section>
                <section class="card section accordion">
                    <div class="accordion-head" onclick="toggleSection(this)"><div><div class="section-title">Telemetry</div><div class="section-sub">Signals, refresh, logs</div></div><span class="status-chip">DATA</span></div>
                    <div class="accordion-body">{logs_html}</div>
                </section>
                <section class="card section accordion">
                    <div class="accordion-head" onclick="toggleSection(this)"><div><div class="section-title">Recent activity</div><div class="section-sub">Execution trail</div></div><span class="status-chip">LOG</span></div>
                    <div class="accordion-body"><div class="activity-list">{events_html}</div></div>
                </section>
            </div>
        </div>
    </div>
