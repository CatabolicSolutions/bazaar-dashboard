...
TMP_RAW=$(mktemp)
TMP_BOARD=$(mktemp)
OUTPUT_BOARD_PATH="/home/alfred-deploy/out/tradier_leaders_board.txt"
...
# Also copy to central location if we have sudo access (optional)
sudo cp "$OUTPUT_BOARD_PATH" "/var/www/bazaar/out/tradier_leaders_board.txt" 2>/dev/null || true