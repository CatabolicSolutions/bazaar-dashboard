...
def hq_history():
    notes_path = STATE / 'hq_operator_notes.jsonl'
    notes = []
    if notes_path.exists():
        for line in read_lines(notes_path)[-20:]:
            try:
    ...
    return {
        'ok': True,
        'refresh_status': read_json(STATE / 'refresh_status.json', {}),
        'action_feedback': read_json(STATE / 'action_feedback.json', {}),
        'queue': read_json(STATE / 'queue_state.json', []),
        'notes': notes,
        'events': read_json(STATE / 'events.json', []),
    }
